            ...
            <div class="p-8 space-y-6 bg-slate-50">
                    ...
                    <form wire:submit="updateTag" class="space-y-4">
                        <div class="space-y-1">
                            ...
                            <input type="text" wire:model="editName"
                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 transition-all"
                        ...
                        <div class="space-y-1">
                            ...
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <input type="color" wire:model="editColor"
                                    style="height: 42px; width: 42px; border: 2px solid #e2e8f0; border-radius: 12px; cursor: pointer; padding: 2px;">
                                <input type="text" wire:model="editColor"
                                    class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 transition-all font-mono text-sm"
                                    placeholder="#6366f1">
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <button type="submit"
                                class="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all">
                                Update
                            </button>
                            <button type="button" wire:click="cancelEdit"
                                class="px-4 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-100 transition-all">
                                Cancel
                            </button>
                        </div>
                    </form>
                @else
                    ...
                    <form wire:submit="createTag" class="space-y-4">
                        ...
                        <button type="submit"
                            class="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                            Create Tag
                        </button>
                    </form>
                @endif
            ...
            <div class="p-8 md:col-span-2">
                ...
                @if($tags->isEmpty())
                    <div class="bg-slate-50 p-8 rounded-2xl border border-dashed border-slate-200 text-center">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            style="height: 40px; width: 40px; color: #94a3b8; margin: 0 auto 12px;" fill="none"
                ...
                @else
                    <div style="display: flex; flex-wrap: wrap; gap: 12px;">
                        @foreach($tags as $tag)
                            <div style="display: flex; align-items: center; gap: 10px; padding: 10px 16px; background-color: white; border: 1px solid #e2e8f0; border-radius: 16px; transition: all 0.2s;"
                                class="hover:shadow-md group">
                                <div
                                    style="height: 14px; width: 14px; border-radius: 9999px; background-color: {{ $tag->color }}; flex-shrink: 0;">
                                </div>
